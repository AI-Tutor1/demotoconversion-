 ...
 *   score  → 5-button scale with low/high anchor labels
 *   yesno  → two-pill toggle
 *   choice → select
 *   text   → textarea
 * Every question gets an optional note textarea below. For 'yesno' questions
 * with `requireNoteWhen`, the note shows required-state when the value matches.
 */
interface RubricQuestionProps {
  q: HrInterviewQuestion;
  answer: RubricAnswer;
  onChange: (patch: Partial<RubricAnswer>) => void;
}

function RubricQuestion({ q, answer, onChange }: RubricQuestionProps) {
  ...
  // Keep showNote true once triggered (so the field doesn't disappear mid-edit)
  // and also when a question forces it via requireNoteWhen.
  ...
  return (
    <div style={{
      ...
      {/* Header row: label + answer control inline where possible */}
      ...
      {/* Scale anchor labels (only for score) */}
      ...
      {/* Text-type questions get a textarea on its own row */}
      {q.type === "text" && (
        <textarea
          className="apple-input apple-textarea"
          rows={3}
      ...
      {/* Optional hint row for non-text questions */}
      {q.type !== "text" && q.hint && (
      ...
      {/* Note — collapsed by default; only for non-text questions */}
 ...
 * the current value to clear (matches POUR tag toggle pattern).
 */
function ScoreScale({
  max, value, onChange,
}: {
