...
const ALL_KEYS: (QKey | OtherKey)[] = [
  ...Q_KEYS, "pour_issues", "overall_summary", "improvement_suggestions", "improvement_focus",
];
...
export default function SessionDraftReview({
  sessionId,
  draft,
  transcript,
  onApproved,
  ...
  const [states, setStates] = useState<Record<QKey | OtherKey, FieldState>>(() =>
    ALL_KEYS.reduce((acc, k) => ({ ...acc, [k]: "untouched" }), {} as Record<QKey | OtherKey, FieldState>)
  );
  ...
  const acceptAll = () =>
    setStates(ALL_KEYS.reduce((acc, k) => ({ ...acc, [k]: "accepted" }), {} as Record<QKey | OtherKey, FieldState>));
  ...
  const allDecided = useMemo(() => ALL_KEYS.every((k) => states[k] !== "untouched"), [states]);
  const approvalRate = useMemo(() => {
    const accepted = ALL_KEYS.filter((k) => states[k] === "accepted").length;
    return accepted / ALL_KEYS.length;
  }, [states]);
  const anyEdited = useMemo(() => ALL_KEYS.some((k) => states[k] === "edited"), [states]);

  const submit = async () => {
    if (submitting) return;
    if (!allDecided) {
      const undecided = ALL_KEYS.filter((k) => states[k] === "untouched").length;
      flash(`${undecided} field${undecided === 1 ? "" : "s"} still need Accept or Edit.`);
      return;
    }
    setSubmitting(true);
    try {
      ...
      await supabase
        ...
        .update({
          ...
          draft_data: {
            ...
            score_interpretation: badge.label,
          },
        })
        .eq("id", draft.id);
      // Update session processing_status
      await supabase
        .from("sessions")
        .update({ processing_status: "approved" })
        .eq("id", sessionId);
      flash("Session scorecard approved");
      onApproved?.();
  ...
  const QuestionCard = ({ k }: { k: QKey }) => {
    ...
    return (
      <div style={{ background: "#fff", border: "1px solid #e8e8ed", borderLeft: borderFor(k), borderRadius: 12, padding: "14px 18px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: NEAR_BLACK }}>
              {meta.label}
              <span style={{ fontSize: 11, color: MUTED, fontWeight: 400, marginLeft: 8 }}>{meta.scaleLabel}</span>
              <span style={{ marginLeft: 10, fontSize: 10, color: BLUE, fontWeight: 600, letterSpacing: 0.5 }}>AI DRAFT</span>
            </div>
          </div>
          <ToggleButtons k={k} />
        </div>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
          {locked(k) ? (
            <div style={{ width: 52, height: 52, borderRadius: 10, background: color, color: "#fff", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1 }}>{v.score}</div>
              <div style={{ fontSize: 9, opacity: 0.85, marginTop: 2 }}>/ {meta.max}</div>
            </div>
          ) : (
            <input
              type="number" min={meta.min} max={meta.max} className="apple-input"
              value={v.score}
              onChange={(e) => setValues((prev) => ({ ...prev, [k]: { ...prev[k], score: Number(e.target.value) } }))}
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {locked(k) ? (
              ...
              <textarea className="apple-input apple-textarea" style={{ fontSize: 13, minHeight: 60 }}
                value={v.evidence}
                onChange={(e) => setValues((prev) => ({ ...prev, [k]: { ...prev[k], evidence: e.target.value } }))}
              />
            )}
          </div>
        </div>
    ...
    return (
      <div style={{ background: "#fff", border: "1px solid #e8e8ed", borderLeft: borderFor(k), borderRadius: 12, padding: "14px 18px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <span className="section-label">{label} <span style={{ color: BLUE, marginLeft: 6, letterSpacing: 0.5 }}>AI DRAFT</span></span>
          <ToggleButtons k={k} />
        </div>
        {locked(k) ? (
          ...
          <textarea className="apple-input apple-textarea" value={v}
            onChange={(e) => setValues((prev) => ({ ...prev, [k]: e.target.value }))} rows={3}
          />
        )}
      </div>
    );
  ...
  return (
    <div className="session-scorecard-grid" style={{ display: "grid", gridTemplateColumns: transcript ? "minmax(0, 3fr) minmax(0, 2fr)" : "1fr", gap: 16, maxWidth: 1300, margin: "0 auto" }}>
      {/* LEFT — scorecard */}
      <div>
        {/* Total banner */}
        <div style={{ background: "#fff", border: "1px solid #e8e8ed", borderRadius: 14, padding: "16px 20px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <p className="section-label">Total score</p>
            <div style={{ marginTop: 4, display: "flex", alignItems: "baseline", gap: 10 }}>
              <span style={{ fontSize: 40, fontWeight: 700, lineHeight: 1, color: NEAR_BLACK }}>{totalScore}</span>
          ...
          <button type="button" onClick={acceptAll} className="pill pill-outline" style={{ padding: "6px 14px", fontSize: 13 }}>Accept all</button>
        </div>
        ...
        <div style={{ marginTop: 12 }}>
          <div style={{ background: "#fff", border: "1px solid #e8e8ed", borderLeft: borderFor("pour_issues"), borderRadius: 12, padding: "14px 18px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <span className="section-label">POUR issues <span style={{ color: BLUE, marginLeft: 6, letterSpacing: 0.5 }}>AI DRAFT</span></span>
              <ToggleButtons k="pour_issues" />
            </div>
            {locked("pour_issues") ? (
              values.pour_issues.length === 0 ? (
                <p style={{ fontSize: 13, color: MUTED, fontStyle: "italic", margin: 0 }}>No POUR issues flagged</p>
              ) : (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {values.pour_issues.map((issue, i) => (
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "140px 1fr auto", gap: 8, alignItems: "center" }}>
                    <select className="apple-input" value={issue.category}
                      onChange={(e) => { const updated = values.pour_issues.map((p, j) => j === i ? { ...p, category: e.target.value } : p); setValues((prev) => ({ ...prev, pour_issues: updated })); }}>
                      {POUR_CATS.map((c) => <option key={c} value={c}>{c}</option>)}
                    </select>
                    <input className="apple-input" style={{ fontSize: 13 }} placeholder="Describe the issue..."
                      value={issue.description}
                      onChange={(e) => { const updated = values.pour_issues.map((p, j) => j === i ? { ...p, description: e.target.value } : p); setValues((prev) => ({ ...prev, pour_issues: updated })); }}
                    />
                    <button type="button"
                      onClick={() => setValues((prev) => ({ ...prev, pour_issues: prev.pour_issues.filter((_, j) => j !== i) }))}
                      style={{ fontSize: 18, color: MUTED, background: "none", border: "none", cursor: "pointer", padding: "0 6px", lineHeight: 1 }}>×</button>
                  </div>
                ))}
                <button type="button" className="pill pill-outline" style={{ alignSelf: "flex-start", fontSize: 13, padding: "6px 14px" }}
                  onClick={() => setValues((prev) => ({ ...prev, pour_issues: [...prev.pour_issues, { category: POUR_CATS[0], description: "" }] }))}>
                  + Add issue
                </button>
              </div>
            )}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 20, gap: 12, flexWrap: "wrap" }}>
          <button type="button" onClick={onReject} className="pill pill-outline"
            style={{ padding: "10px 22px", fontSize: 14, color: "#c13030", borderColor: "#c13030" }} disabled={submitting}>
            Reject draft
          </button>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <button type="button" onClick={submit} className="pill pill-blue"
              style={{ padding: "10px 24px", fontSize: 15, opacity: !allDecided || submitting ? 0.5 : 1, cursor: !allDecided || submitting ? "not-allowed" : "pointer" }}
      ...
      {/* RIGHT — transcript */}
      {transcript && (
        <div style={{ background: LIGHT_GRAY, borderRadius: 16, padding: "16px 18px", height: "calc(100vh - 180px)", overflowY: "auto", position: "sticky", top: 92 }}>
          <p className="section-label" style={{ marginBottom: 10 }}>Transcript</p>
          <pre style={{ fontSize: 12, lineHeight: 1.55, whiteSpace: "pre-wrap", wordBreak: "break-word", fontFamily: "ui-monospace, Menlo, monospace", color: NEAR_BLACK, margin: 0 }}>
            {transcript}
          </pre>
        </div>
      )}
