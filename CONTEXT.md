...
**What can be deleted:**
- **Demos** — anywhere a demo is listed or shown. As of 2026-04-25 this means: dashboard `/` Recent demos, `/conducted`, `/drafts` (AI queue + Sales queue), `/sales` (queue card + detail panel + **bulk action bar**), and `/analyst/{id}` detail. Cascades via `ON DELETE CASCADE` to `pour_issues`, `demo_drafts`, `demo_accountability`, and `task_queue`. The /sales bulk bar is manager-only like the rest; it opens one confirm modal for N-selected demos via `confirmBulkDeleteDemos` and the `setDemos` diff-writer batches the DELETE into a single `.in('id', [...])` call.
...
**How it runs in code:** Every delete surface calls one of three helpers on `useStore()` — `confirmDeleteDemo(demo, { onAfterDelete? })`, `confirmBulkDeleteDemos(demos, { onAfterDelete? })` (for multi-select bulk), or `confirmDeleteSession(id, label, { onAfterDelete? })`. The helper opens the shared confirm modal, on OK performs the DB delete (via `setDemos` diff-writer for demos — single or batched, direct `supabase.from("sessions").delete()` for sessions), appends an `activity_feed` entry `"deleted"`, and flashes "Demo deleted" / "N demos deleted" / "Session deleted". Direct `supabase.from(...).delete()` calls from pages are banned — see CLAUDE.md Do-Not rules.
